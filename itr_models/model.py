"""
itr_models/model.py
提供跨模态检索的 GNN + Attention + 对比 loss 实现
负责：
图结构构建
图卷积 / 注意力传递
图像/文本特征编码
对比损失计算
生成跨模态相似度输出
所有这些功能都会在训练和预测阶段被 Training 类调用。
"""
import torch
import math
...
# 实现经典 GCN（图卷积网络），输入节点特征 + 邻接矩阵 → 输出节点表示
...
#实现 GAT（Graph Attention Network）层，使节点间的关系信息由自注意力机制动态学习权重，而不是简单邻接权重
...
# 对多种图结构结果进行 Attention 融合（例如特定结构和特征相似度图）——尤其用于融合图卷积输出。
...
# 组合结构图与特征图上 GCN 输出，并用 Attention 合并三种嵌入。（结构 GCN、特征 GCN、两者平均）
...
# 这个类是 ASCMR 跨模态检索主模型（Deep Active Learning + GNN）
...
class DALGNN(nn.Module):
    ...
    def forward(self, feature_img, feature_text):
        ...
        layers = []
        # 这里 x 是类别 embedding（传入的图像/文本语义先验），构造邻接矩阵用于图神经
        x = self.inp
        ...
        # 最终得到得到每类共享 embedding：将图像/文本表示和类别 embedding 做内积（相似度）用作分类或检索 logits。
        x = F.normalize(x, dim=1)
