...
def dual_softmax_loss(sim: torch.Tensor, sim_t: torch.Tensor = None, y: torch.Tensor = None) -> torch.Tensor:
    """
    恢复纯净的 Dual Softmax Loss（坚决不注入样本权重，保护特征流形）
    """
    sim_i2t = F.softmax(sim, dim=1).diag() + 1e-8
    if sim_t is None:
        sim_t = sim.t()
    sim_t2i = F.softmax(sim_t, dim=0).diag() + 1e-8

    loss_i2t = -torch.log(sim_i2t)
    loss_t2i = -torch.log(sim_t2i)

    return ((loss_i2t + loss_t2i) / 2.0).mean()
