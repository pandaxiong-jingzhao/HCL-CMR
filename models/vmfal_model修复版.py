...
class VMFUncertaintyModel:
    ...
    def compute_training_weights(
        ...
        相比原版的两处改进：
        1. pool_factor：标注池小时（<2000）自动缩小scale
           - Flickr25K 前中期标注少，scale被大幅压低，避免前期拖累
           - COCO/NUS 标注池增长快，Round 3后pool_factor=1.0，几乎无影响

        2. round_factor：前30%轮次中scale从0渐进增长（二次方）
           - 前期模型不稳定时加权更温和
           - 后期恢复原始scale
        """
        N = labels.shape[0]
        ...
        # =====================================================
        # 改进1：基于当前标注量的 scale 自适应
        # =====================================================
        pool_threshold = 2000.0
        pool_factor = min(1.0, N / pool_threshold)

        # =====================================================
        # 改进2：前期轮次渐进加权
        # =====================================================
        warmup_end = max(total_rounds * 0.3, 3.0)

        if R < warmup_end:
            ramp = R / warmup_end
            round_factor = ramp * ramp
        else:
            round_factor = 1.0

        # 综合 effective_scale
        effective_scale = scale * pool_factor * round_factor

        # =====================================================
        # 以下为原版逻辑，仅将 scale 替换为 effective_scale
        # =====================================================
        ...
        decay = effective_scale * age / max(R, 1.0)
        w = (1.0 - decay).clamp(min=max(0.5, 1.0 - effective_scale), max=1.0)
        w = w / (w.mean() + 1e-8)

        n_decayed = int((w < 0.99).sum())
        print(f"  [W] round={round_id} N={N} scale={scale:.3f} "
              f"pool_factor={pool_factor:.2f} round_factor={round_factor:.2f} "
              f"eff_scale={effective_scale:.4f} "
